...
Tree* erase(Tree* T, const int key) {
    ...
    if (key == T->key) {
        ...
        if (T->left != nullptr && T->right != nullptr){ //Fall4c
            int x = 0;
            int& y = x;
            Tree* newRight = extractMin(T->right, y);
            Tree* newTree = new Tree(T->left, x, newRight);
            T->right = nullptr;
            T->left = nullptr;
            delete T;
            return newTree;
...
int main() {
    ...
    while(std::cin >> c >> key) {
        ...
        case 'e':
        {
            ...
            int& r = min;
            T = extractMin(T, r);
            if (print) std::cout << r << ' ' << T << std::endl;
            break;
        }
